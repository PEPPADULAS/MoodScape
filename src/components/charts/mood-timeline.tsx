...
export default function MoodTimeline({ className = "" }: MoodTimelineProps) {
  ...
  const getScales = () => {
    if (moodData.length === 0) return { xScale: () => 0, yScale: () => 0, dates: [], maxCount: 0 }
    ...
    return { xScale, yScale, dates, maxCount }
  }
        ...
        <div className="relative">
          ...
          <svg width={width} height={height} className="overflow-visible">
            ...
            {Array.from({ length: maxCount + 1 }, (_, i) => (
              <g key={i}>
                <text
                  x={padding.left - 10}
                  y={padding.top + yScale(maxCount - i) + 5}
                  textAnchor="end"
                ...
                <line
                  x1={padding.left}
                  y1={padding.top + yScale(maxCount - i)}
                  x2={width - padding.right}
                  y2={padding.top + yScale(maxCount - i)}
                  stroke="currentColor"
                  strokeWidth="0.5"
                  opacity="0.2"
                />
            ...
            {/* X-axis labels */}
            {dates.filter((_, i) => i % 3 === 0).map((date, i) => (
              <text
                key={date}
                x={padding.left + xScale(date)}
          ...
          {hoveredPoint && (
            <motion.div
              ...
              style={{
                left: padding.left + xScale(hoveredPoint.date) + 10,
                top: padding.top + yScale(hoveredPoint.count) - 10,
              }}
            >
